...
public class MessageService(IPublishEndpoint publishEndpoint, ILogger<MessageService> logger) : IMessageService
{
    ...
    {
        logger.LogInformation("Attempting to publish SubscriptionCanceledEvent for {SubscriptionId}", canceledSubscription.Id);
        
        var publishedNotification = publishEndpoint.Publish(canceledSubscription, cancellationToken);
        logger.LogInformation("Successfully published SubscriptionCanceledEvent for {SubscriptionId}",canceledSubscription.Id);
    ...
    {
        logger.LogInformation("Attempting to publish SubscriptionRenewedEvent for {SubscriptionId}", renewedSubscription.Id);
        
        var publishedNotification = publishEndpoint.Publish(renewedSubscription, cancellationToken);
        logger.LogInformation("Successfully published SubscriptionRenewedEvent for {SubscriptionId}", renewedSubscription.Id);
